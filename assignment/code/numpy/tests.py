...
def t2(X):
    ...
    """
    vals, vecs = np.linalg.eig(X); return vecs[:, np.argmin(vals)]
...
def t6(N):
    ...
    Instructor: 5 lines
    """
    A = np.zeros((N, N)); A[5:-5, 5:-5] = 1; return A
...
def t7(X):
    ...
    """
    n = np.linalg.norm(X, axis=1, keepdims=True); return X / np.where(n == 0, 1, n)
...
def t8(X):
    ...
    """
    m = X.mean(axis=1, keepdims=True); s = X.std(axis=1, keepdims=True); return (X - m) / np.where(s == 0, 1, s)
...
def t9(q, k, v):
    ...
    """
    w = np.exp(-np.sum((k - q)**2, axis=1, keepdims=True)); return np.sum(w * v)
...
def t10(Xs):
    ...
    5) Our 3-line solution uses no loops, and uses the algebraic trick from the
       next problem.
    """
    C = np.mean(np.stack(Xs, axis=0), axis=1)
    ss = np.sum(C**2, axis=1, keepdims=True)
    D2 = ss + ss.T - 2 * (C @ C.T)
    return np.sqrt(np.maximum(0, D2))
...
def t12(X, Y):
    ...
    """
    sx = np.sum(X**2, axis=1, keepdims=True); sy = np.sum(Y**2, axis=1, keepdims=True).T
    D2 = sx + sy - 2 * (X @ Y.T)
    return np.sqrt(np.maximum(0, D2))
...
def t13(q, V):
    ...
    Hint: np.argmax
    """
    return np.array(np.argmax(q @ V.T))
...
def t18(N, r, x, y):
    ...
    return (np.sqrt((Xg - x)**2 + (Yg - y)**2) < r).astype(float)
...
def t19(N, s, x, y):
    ...
    """
    Xg, Yg = np.meshgrid(np.arange(N), np.arange(N))
    return np.exp(-((Xg - x)**2 + (Yg - y)**2) / (s**2))
...
def t20(N, v):
    ...
    a, b, c = v
    Xg, Yg = np.meshgrid(np.arange(N), np.arange(N))
    return np.abs(a * Xg + b * Yg + c) / np.sqrt(a*a + b*b)
